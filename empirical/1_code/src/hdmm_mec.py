...
class mechanism:
    ...
    def set_strategy(self,strategy_path):
        
        with open(strategy_path,'rb') as f:
            self.A = pickle.load(f)['p-Identity']
    ...
    def laplace(self,x,eps=np.sqrt(2),trials=1,seed=100):
        ...
        delta = self.A.sensitivity()
        A = self.A.sparse_matrix()
        ys = [A.dot(x)+prng.laplace(loc=0,scale=delta/eps,size=A.shape[0]) for i in range(trials)]
        return ys
    ...
    def post_process(self,ys,integer=True,W_test = None):
        ...
        for y in ys:
            ...
            if self.engine == 'ls':
                ...
                print('does not exist such engine')
                break
    ...
    def run(self,columns,epsilons,engine = 'ls-round',trials=1,seed=100,integer=True):
